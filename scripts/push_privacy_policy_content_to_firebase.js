#!/usr/bin/env node
/**
 * Push privacy policy screen content to Firebase Firestore
 * Uses Firebase CLI's stored credentials (no service account key needed)
 *
 * Usage: node scripts/push_privacy_policy_content_to_firebase.js
 *
 * This pushes privacy_policy_screen_content.json to:
...
const JSON_FILE = path.resolve(__dirname, '..', 'assets', 'data', 'firebase', 'privacy_policy_screen_content.json');
const COLLECTION = 'privacy_policy_screen_content';
const DOC_ID = 'data';
...
async function main() {
  console.log('=== Push Privacy Policy Screen Content to Firebase ===\n');
  ...
  try {
    ...
    if (existing.fields && existing.fields.privacy_policy_screen_content_version) {
      const vField = existing.fields.privacy_policy_screen_content_version;
  ...
  const versionDoc = toFirestoreDoc({
    privacy_policy_screen_content_version: currentVersion + 1,
  ...
  console.log('\n=== All done! Privacy policy screen content pushed to Firebase. ===');
