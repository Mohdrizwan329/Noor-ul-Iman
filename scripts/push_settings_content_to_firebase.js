#!/usr/bin/env node
/**
 * Push settings screen content to Firebase Firestore
 * Uses Firebase CLI's stored credentials (no service account key needed)
 *
 * Usage: node scripts/push_settings_content_to_firebase.js
 *
 * This pushes settings_screen_content.json to:
...
const JSON_FILE = path.resolve(__dirname, '..', 'assets', 'data', 'firebase', 'settings_screen_content.json');
const COLLECTION = 'settings_screen_content';
const DOC_ID = 'data';
...
async function main() {
  console.log('=== Push Settings Screen Content to Firebase ===\n');
  console.log(`Reading data from: ${JSON_FILE}`);
  const data = JSON.parse(fs.readFileSync(JSON_FILE, 'utf8'));

  console.log(`Data loaded:`);
  console.log(`  - Strings: ${data.strings ? Object.keys(data.strings).length : 0}`);
  console.log(`  - Languages: ${data.languages ? data.languages.length : 0}`);
  console.log(`  - Default user names: ${data.default_user_names ? data.default_user_names.length : 0}`);
  console.log(`  - City translations: ${data.city_translations ? Object.keys(data.city_translations).length : 0}`);
  console.log(`  - Country translations: ${data.country_translations ? Object.keys(data.country_translations).length : 0}`);
  console.log(`  - Name transliterations: ${data.name_transliterations ? Object.keys(data.name_transliterations).length : 0}`);
  ...
  try {
    ...
    if (existing.fields && existing.fields.settings_screen_content_version) {
      const vField = existing.fields.settings_screen_content_version;
  ...
  const versionDoc = toFirestoreDoc({
    settings_screen_content_version: currentVersion + 1,
  ...
  console.log('\n=== All done! Settings screen content pushed to Firebase. ===');
  console.log(`Collection: ${COLLECTION}`);
  console.log(`Document: ${DOC_ID}`);
  console.log(`Total strings: ${Object.keys(data.strings).length}`);
  console.log(`Total languages: ${data.languages.length}`);
  console.log(`Total city translations: ${Object.keys(data.city_translations).length}`);
  console.log(`Total country translations: ${Object.keys(data.country_translations).length}`);
  console.log(`Total name transliterations: ${Object.keys(data.name_transliterations).length}`);
  console.log(`Supported languages: English, Urdu, Hindi, Arabic`);
}
