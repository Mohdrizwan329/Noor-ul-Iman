#!/usr/bin/env node
/**
 * Push UI translations to Firebase Firestore
 * Uses Firebase CLI's stored credentials (no service account key needed)
 *
 * Usage: node scripts/push_ui_translations_to_firebase.js
 *
 * This pushes ui_translations.json to:
 *   Collection: ui_translations
 *   Document: all_translations
 */
...
const JSON_FILE = path.resolve(__dirname, '..', 'assets', 'data', 'firebase', 'ui_translations.json');
const COLLECTION = 'ui_translations';
const DOC_ID = 'all_translations';
...
async function main() {
  console.log('=== Push UI Translations to Firebase ===\n');
  ...
  for (const lang of languages) {
    console.log(`  - ${lang}: ${Object.keys(data.translations[lang]).length} keys`);
  }
  ...
  try {
    ...
    if (existing.fields && existing.fields.ui_translations_version) {
      const vField = existing.fields.ui_translations_version;
  ...
  const versionDoc = toFirestoreDoc({
    ui_translations_version: currentVersion + 1,
  ...
  console.log('\n=== All done! UI translations pushed to Firebase. ===');
