#!/usr/bin/env node
/**
 * Push about screen content to Firebase Firestore
 * Uses Firebase CLI's stored credentials (no service account key needed)
 *
 * Usage: node scripts/push_about_content_to_firebase.js
 *
 * This pushes about_screen_content.json to:
...
const JSON_FILE = path.resolve(__dirname, '..', 'assets', 'data', 'firebase', 'about_screen_content.json');
const COLLECTION = 'about_screen_content';
const DOC_ID = 'data';
...
async function main() {
  console.log('=== Push About Screen Content to Firebase ===\n');
  ...
  try {
    ...
    if (existing.fields && existing.fields.about_screen_content_version) {
      const vField = existing.fields.about_screen_content_version;
  ...
  const versionDoc = toFirestoreDoc({
    about_screen_content_version: currentVersion + 1,
  ...
  console.log('\n=== All done! About screen content pushed to Firebase. ===');
